...
const saveCanvasToIPFS = async (blob, fileExtension) => {
  const queryParams = new URLSearchParams(window.location.search);
  const id = queryParams.get('id');
  if (id) {
    console.log(blob, fileExtension);
  }
  window.location.href =
    'https://epic-clarke-aadfbe.netlify.app/mint.html?nftId=1';
};
...
function renderFrames(settings) {
  ...
  gif.pipe(blobStream()).on('finish', function() {
    saveCanvasToIPFS(this.toBlob(), 'gif');
  });
